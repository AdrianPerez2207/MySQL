...
select * 
from alumno
where year(fecha_nacimiento) like "1999";
...
inner join alumno_se_matricula_asignatura as alumno on a.id=alumno.id_asignatura
inner join curso_escolar as c on alumno.id_curso_escolar=c.id
...
from alumno as a
inner join alumno_se_matricula_asignatura as matriculado on a.id=matriculado.id_alumno
inner join curso_escolar as c on matriculado.id_curso_escolar=c.id
inner join asignatura on matriculado.id_asignatura=asignatura.id
inner join grado as g on asignatura.id_grado = g.id
...
select distinct asignatura.nombre as asignatura, c.anyo_inicio as "año inicio", c.anyo_fin as "año fin"
from curso_escolar as c
inner join alumno_se_matricula_asignatura as matriculado on c.id=matriculado.id_curso_escolar
inner join alumno as a on matriculado.id_alumno=a.id
inner join asignatura on matriculado.id_asignatura=asignatura.id
where a.nif like "26902806M";
